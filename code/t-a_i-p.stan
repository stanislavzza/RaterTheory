...
functions {
  ...
  real log_lik(real rate, real acc, real truth, real pop){
    ...
    cond_prob = acc*truth + (1.0 - acc)*pop;
...
parameters {
 ...
 // real<lower=0, upper = 1> truth[S];  // random truth value for each subject
  real<lower=0, upper = 1> P; // guess rate for ~ Class 1
   real<lower=0, upper = 1> T; // true rate of class 1
}
...
//  }
  ...
  // Likelihood model
  for(i in 1:N) {
      target += log_lik(rating[i], accuracy[rater_index[i]], T, P);
  }
}
